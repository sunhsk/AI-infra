...
int main(void){
    ...
    while (numElements > 1) {
        ...
        reduce_v3<<<blocksPerGrid, threadsPerBlock, threadsPerBlock * sizeof(float)>>>(d_in, d_out, numElements);  //threadsPerBlock * sizeof(float) 是 CUDA kernel 启动时指定的动态共享内存大小，单位是字节。每个 block 分配的动态共享内存大小
